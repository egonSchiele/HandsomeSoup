...
-- | Example: contents <- runMaybeT $ openUrl "http://foo.com"
...
-- | Example: doc <- fromUrl "http://foo.com"
fromUrl :: String -> IO (IOSArrow XmlTree (NTree XNode))
fromUrl url = do
  contents <- runMaybeT $ openUrl url
  return $ parseHtml (fromMaybe "" contents)

-- | Given a string, parses it and returns a document
-- | Example: doc = parseHtml "<h1>hello!</h1>"
...
-- | Example: doc >>> css "a" ! "href"
(!) :: ArrowXml cat => cat a XmlTree -> String -> cat a String
(!) a str = a >>> getAttrValue str

-- | A css selector for getting elements from a document.
-- | Example: doc >>> css "#menu li"
